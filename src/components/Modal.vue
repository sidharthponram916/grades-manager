<template lang="">
  <div v-if="show" class="modal">
    <div
      class="pt-2 pb-2 m-auto rounded w-1/2 text-xl bg-slate-100 text-center"
    >
      <div class="text-right mr-7" @click="this.$emit('close')">x</div>
      ...
      <table class="w-11/12 text-sm mt-5 rounded mb-5 m-auto bg-slate-200">
        <thead>
          <tr>
            <td class="pt-3 font-bold">Name</td>
            <td class="pt-3 font-bold">Category</td>
            <td class="pt-3 font-bold">Grade</td>
          </tr>
        </thead>
        <tbody>
          <tr v-for="assignment in course.assignments" :key="assignment.name">
            <td class="pt-3 font-medium">{{ assignment.name }}</td>
            <td class="pt-3 font-medium">{{ assignment.category }}</td>
            <td class="pt-3 font-medium">
              <div
          ...
          <tr>
            ...
            <td class="pt-3 pb-3 font-medium">
              ...
              >
                ...
                <option v-for="a in Object.keys(course.weights)">
                  {{ a }}
                </option>
              </select>
            </td>
  ...
  methods: {
    ...
    calculateAverage() {
      ...
      for (let i = 0; i < this.course.assignments.length; i++) {
        ...
        let weight = this.course.weights[assignment.category];
...
.modal {
  display: block; /* Show by default when the state is true */
